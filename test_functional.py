...
def test_main_car():
    ...
    assert core.scene.car_m.health == 100, "Main character's health must be equal 100 in the begin"
    assert hasattr(core.scene.car_m, 'controller'), "Main car must have a controller"
    assert isinstance(core.scene.car_m.controller, KeyboardController), "Controller must be keyboard"
...
def test_object_factory():
    ...
    assert car.car_model.body in core.scene.space.bodies, 'Car must be added to the physics scene'
    ...
    assert isinstance(cone, MovableObstacle), 'Object factory must return movable obstacle'
    ...
    assert cone.obstacle_model.body in core.scene.space.bodies, 'Cone must be added to the physics scene'
    ...
    for model in ['tree', 'rubbish_line', 'metal_pipe', 'x_barrier', 'y_barrier']:
        ...
        assert isinstance(obj, StaticObstacle), 'Object factory must return static obstacle'
        ...
        assert obj.obstacle_model.body in core.scene.space.bodies, f'{model} must be added to the physics scene'
...
def test_scene_setup():
    ...
    for cone in core.scene.traffic_cones:
        assert isinstance(cone, MovableObstacle), 'Only cones should be in list with cones'
